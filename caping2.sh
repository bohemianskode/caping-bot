...
-H "token: a16d883a09aa4dd38d0d51aea5431a2b" \
...
oke=$(echo -e "$O" | jq -r .message)
o0=$(echo -e "$O" | jq -r .code)
echo -e "SHARE  _ code : $0 | message :$oke"
return
}
video(){
code=$(echo $RANDOM)
...
oke=$(echo -e "$O" | jq -r .message)
o0=$(echo -e "$O" | jq -r .code)
echo -e "VIDEO  _ code : $o0 | message :$oke"
return
}
news(){
...
oke=$(echo -e "$O" | jq -r .message)
o0=$(echo -e "$O" | jq -r .code)
echo -e "NEWS   _ code : $o0 | message :$oke"
return
}
klik(){
#74039409
...
echo -e "KLIK   _ code : $o0 | message :$oke"
return
}
#echo -n "UID :"; read uid
#echo -n "N ID :"; read n
...
oke=$(echo -e "$O" | jq -r .message)
o0=$(echo -e "$O" | jq -r .code)
echo -e "ARTKL  _ code : $o0 | message :$oke
return
}
getcoin(){
...
-H "Cookie: u=`cat uzer.txt| cut -d "|" -f1`;n=`cat user.txt| cut -d "|" -f2`" \
-H "X-Requested-With: XMLHttpRequest" \
-H "Host: ai2.caping.co.id" \
-H "Connection: Keep-Alive" -s)
coin=$(echo -e "$oke" | grep -Po '(?<="coin":)(.*?)(?=})' | cut -d "," -f1)
}
echo -ne "${normal}Masukan Jumlah (1 s/d 9999) > "; read jumlah
count=1;
